...
public class JdbcRedemptionCodeRepository implements RedemptionCodeRepository {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    RedemptionCode saved = new RedemptionCode(
                            id,
                            code.code(),
                            code.productId(),
                            code.guildId(),
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                for (RedemptionCode code : codes) {
                    ...
                    try (ResultSet rs = stmt.executeQuery()) {
                        if (rs.next()) {
                            long id = rs.getLong("id");
                            savedCodes.add(new RedemptionCode(
                                    id,
                                    code.code(),
                                    code.productId(),
                                    code.guildId(),
    ...
    public RedemptionCode update(RedemptionCode code) {
        ...
        String sql = "UPDATE redemption_code SET " +
    ...
    public long countRedeemedByProductId(long productId) {
        String sql = "SELECT COUNT(*) FROM redemption_code WHERE product_id = ? AND redeemed_by IS NOT NULL";
    ...
    public long countUnusedByProductId(long productId) {
        String sql = "SELECT COUNT(*) FROM redemption_code WHERE product_id = ? AND redeemed_by IS NULL";
