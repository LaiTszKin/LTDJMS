...
public class AdminProductPanelHandler extends ListenerAdapter {
    ...
    public AdminProductPanelHandler(ProductService productService,
    ...
    public void onButtonInteraction(ButtonInteractionEvent event) {
        ...
        LOG.debug("Processing product panel button: buttonId={}, userId={}",
    ...
    private void handleProductSelect(StringSelectInteractionEvent event) {
        ...
        productService.getProduct(productId).ifPresentOrElse(
                product -> {
                    productSessions.put(sessionKey, new ProductSessionState(productId, ProductView.DETAIL, 1));
    ...
    private void showProductDetail(ButtonInteractionEvent event) {
        ...
        productService.getProduct(session.productId).ifPresentOrElse(
                product -> {
    ...
    private MessageEmbed buildProductDetailEmbed(Product product) {
        ...
        // Reward info
        if (product.hasReward()) {
            String rewardTypeName = switch (product.rewardType()) {
                case CURRENCY -> "貨幣";
                case TOKEN -> "代幣";
            };
    ...
    private List<ActionRow> buildProductDetailComponents(Product product, RedemptionCodeRepository.CodeStats stats) {
        return List.of(
                ActionRow.of(
    ...
    private void openCreateProductModal(ButtonInteractionEvent event) {
        TextInput nameInput = TextInput.create("name", "商品名稱", TextInputStyle.SHORT)
    ...
    private void handleCreateProductModal(ModalInteractionEvent event) {
        ...
        adminPanelSessionManager.updatePanel(guildId, adminId,
                hook -> refreshProductListView(hook, guildId, adminId));

        event.reply(String.format("✅ 商品「%s」建立成功！", product.name()))
                .setEphemeral(true)
                .queue();
    }
    ...
    private void openGenerateCodesModal(ButtonInteractionEvent event) {
        ...
        TextInput countInput = TextInput.create("count", "生成數量", TextInputStyle.SHORT)
    ...
    private void showCodeList(ButtonInteractionEvent event, int page) {
        ...
        productService.getProduct(session.productId).ifPresentOrElse(
                product -> {
                    RedemptionService.CodePage codePage = redemptionService.getCodePage(
                            session.productId, page, PAGE_SIZE);
    ...
    private List<ActionRow> buildCodeListComponents(RedemptionService.CodePage codePage) {
        ...
        if (codePage.hasPreviousPage()) {
            navButtons.add(Button.secondary(
     ...
     */
    void setProductSessionForTest(long adminId, long guildId, ProductView view, long productId, int page) {
        productSessions.put(getSessionKey(adminId, guildId), new ProductSessionState(productId, view, page));
    }
    ...
    public void refreshProductPanels(long guildId) {
        ...
        productSessions.forEach((key, state) -> {
            ...
            adminPanelSessionManager.updatePanel(guildId, adminId, hook -> {
                try {
                    ...
                    LOG.warn("Failed to refresh product panel for adminId={}, guildId={}", adminId, guildId, e);
                }
            });
        });
    }

    private void refreshProductDetailView(InteractionHook hook, long guildId, long adminId, ProductSessionState state) {
        productService.getProduct(state.productId).ifPresentOrElse(
                product -> {
                    var stats = redemptionService.getCodeStats(product.id());
                    hook.editOriginalEmbeds(buildProductDetailEmbed(product))
                            .setComponents(buildProductDetailComponents(product, stats))
                            .queue(
                                    msg -> LOG.trace("Refreshed product detail for adminId={}, guildId={}", adminId, guildId),
                                    err -> LOG.warn("Failed to edit product detail message for adminId={}, guildId={}", adminId, guildId, err)
                            );
                },
    ...
    private void refreshCodeListView(InteractionHook hook, long guildId, long adminId, ProductSessionState state) {
        productService.getProduct(state.productId).ifPresentOrElse(
                product -> {
                    ...
                    hook.editOriginalEmbeds(buildCodeListEmbed(product, codePage))
                            ...
                            .queue(
                                    msg -> LOG.trace("Refreshed code list for adminId={}, guildId={}", adminId, guildId),
                                    err -> LOG.warn("Failed to edit code list message for adminId={}, guildId={}", adminId, guildId, err)
                            );
                },
                () -> refreshProductListView(hook, guildId, adminId)
        );
    ...
    private void refreshProductListView(InteractionHook hook, long guildId, long adminId) {
        ...
        hook.editOriginalEmbeds(buildProductListEmbed(products))
                ...
                .queue(
                        ...
                        err -> LOG.warn("Failed to edit product list message for adminId={}, guildId={}", adminId, guildId, err)
                );
    }
    ...
    private List<ActionRow> buildProductListComponents(List<Product> products) {
        ...
        StringSelectMenu.Builder menuBuilder = StringSelectMenu.create(SELECT_PRODUCT)
                .setPlaceholder("選擇商品查看詳情");
        ...
        for (Product product : products) {
            ...
            String description = product.hasReward()
                    ? product.formatReward()
                    : "無自動獎勵";
