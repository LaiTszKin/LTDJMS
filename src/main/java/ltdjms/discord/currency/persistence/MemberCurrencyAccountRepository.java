...
public interface MemberCurrencyAccountRepository {
    ...
    /**
     * Adjusts the balance atomically with explicit error handling via Result.
     * Returns Ok with the updated account, or Err with a DomainError if:
     * - The operation would result in a negative balance (INSUFFICIENT_BALANCE)
     * - A database error occurred (PERSISTENCE_FAILURE)
     */
    Result<MemberCurrencyAccount, DomainError> tryAdjustBalance(long guildId, long userId, long amount);
