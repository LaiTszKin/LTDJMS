...
/**
 * Represents a member's game token account within a specific Discord guild.
 * Each member has exactly one game token account per guild with a non-negative balance.
 * Game tokens are independent from the currency system and used to participate in games.
 */
public record GameTokenAccount(
    ...
    /**
     * Creates a new account instance with an adjusted token balance.
