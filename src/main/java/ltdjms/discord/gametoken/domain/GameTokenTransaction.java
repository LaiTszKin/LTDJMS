...
/**
 * Represents a single game token transaction in a user's account history. Each transaction records
 * the amount changed, the resulting balance, and the source of the transaction.
 *
 ...
 */
public record GameTokenTransaction(
    Long id,
    long guildId,
    long userId,
    long amount,
    ...
    String description,
    Instant createdAt) {
  /** Sources of game token transactions. */
  public enum Source {
    /** Tokens added by an administrator via /game-token-adjust or admin panel */
    ADMIN_ADJUSTMENT("管理員調整"),
    /** Tokens deducted for playing dice-game-1 */
    DICE_GAME_1_PLAY("骰子遊戲 1 消耗"),
    /** Tokens deducted for playing dice-game-2 */
    DICE_GAME_2_PLAY("骰子遊戲 2 消耗"),
    /** Generic game consumption (for future games) */
    GAME_PLAY("遊戲消耗"),
   ...
   */
  public static GameTokenTransaction create(
      long guildId,
      long userId,
      long amount,
      long balanceAfter,
    ...
    return new GameTokenTransaction(
        null, guildId, userId, amount, balanceAfter, source, description, Instant.now());
  }
  ...
  public String formatForDisplay() {
    String amountStr = amount >= 0 ? String.format("+%,d", amount) : String.format("%,d", amount);
    String descStr = description != null && !description.isBlank() ? " - " + description : "";
    return String.format(
        "%s | %s | 餘額: %,d%s", source.getDisplayName(), amountStr, balanceAfter, descStr);
  }
