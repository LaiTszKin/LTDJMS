...
/**
 * Repository interface for game token transactions.
 * Provides methods to save and query transaction history.
 */
public interface GameTokenTransactionRepository {
     ...
     */
    GameTokenTransaction save(GameTokenTransaction transaction);
     ...
     */
    List<GameTokenTransaction> findByGuildIdAndUserId(long guildId, long userId, int limit, int offset);
    ...
    /**
     * Deletes all transactions for a user in a guild.
     * Used primarily for testing or account cleanup.
     *
     * @param guildId the Discord guild ID
     * @param userId  the Discord user ID
