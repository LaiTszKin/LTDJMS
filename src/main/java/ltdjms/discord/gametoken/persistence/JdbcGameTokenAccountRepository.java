...
/**
 * JDBC-based implementation of GameTokenAccountRepository.
 * Provides methods to load, create, and update token balances atomically while preventing negative values.
 */
public class JdbcGameTokenAccountRepository implements GameTokenAccountRepository {
    ...
    public Optional<GameTokenAccount> findByGuildIdAndUserId(long guildId, long userId) {
        String sql = "SELECT guild_id, user_id, tokens, created_at, updated_at " +
    ...
    public GameTokenAccount save(GameTokenAccount account) {
        String sql = "INSERT INTO game_token_account (guild_id, user_id, tokens, created_at, updated_at) " +
                "VALUES (?, ?, ?, ?, ?)";
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            LOG.info("Saved game token account: guildId={}, userId={}, tokens={}",
                    account.guildId(), account.userId(), account.tokens());
            return account;

        } catch (SQLException e) {
            LOG.error("Failed to save game token account for guildId={}, userId={}", account.guildId(), account.userId(), e);
    ...
    public GameTokenAccount setTokens(long guildId, long userId, long newTokens) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    GameTokenAccount updated = mapRow(rs);
                    LOG.info("Set game tokens: guildId={}, userId={}, newTokens={}",
                            guildId, userId, updated.tokens());
                    return updated;
                } else {
    ...
    public boolean deleteByGuildIdAndUserId(long guildId, long userId) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            LOG.error("Failed to delete game token account for guildId={}, userId={}", guildId, userId, e);
