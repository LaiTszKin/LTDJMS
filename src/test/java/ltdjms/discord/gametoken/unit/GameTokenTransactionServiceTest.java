...
class GameTokenTransactionServiceTest {
    ...
    class GetTransactionPage {
        ...
        void shouldReturnCorrectPageForFirstPage() {
            // Given
            List<GameTokenTransaction> transactions = List.of(
        ...
        void shouldHandleEmptyResult() {
            ...
            when(transactionRepository.findByGuildIdAndUserId(anyLong(), anyLong(), anyInt(), anyInt()))
                    .thenReturn(List.of());
            when(transactionRepository.countByGuildIdAndUserId(anyLong(), anyLong()))
                    .thenReturn(0L);
    ...
    class RecordTransaction {
        ...
        void shouldSaveTransaction() {
            ...
            GameTokenTransaction saved = new GameTokenTransaction(
                    ...
                    GameTokenTransaction.Source.ADMIN_ADJUSTMENT, "Test", Instant.now());
            when(transactionRepository.save(any(GameTokenTransaction.class))).thenReturn(saved);

            // When
            GameTokenTransaction result = service.recordTransaction(
                    TEST_GUILD_ID, TEST_USER_ID, 50, 150,
                    GameTokenTransaction.Source.ADMIN_ADJUSTMENT, "Test");
    ...
    private GameTokenTransaction createTransaction(long id, long amount, long balanceAfter,
                                                    GameTokenTransaction.Source source) {
        return new GameTokenTransaction(
                id, TEST_GUILD_ID, TEST_USER_ID, amount, balanceAfter,
                source, null, Instant.now());
    }
}
