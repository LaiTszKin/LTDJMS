...
class DefaultBalanceServiceCacheTest {
    ...
    void setUp() {
        ...
        service = new DefaultBalanceService(
                accountRepository,
                configRepository,
                cacheService,
                cacheKeyGenerator
    ...
    class CacheMissTests {
        ...
        void shouldQueryDatabaseAndPopulateCacheOnMiss() {
            // Given
            MemberCurrencyAccount account = MemberCurrencyAccount.createNew(GUILD_ID, USER_ID).withAdjustedBalance(BALANCE);
        ...
        void shouldReturnCorrectBalanceFromDatabaseOnMiss() {
            ...
            MemberCurrencyAccount account = MemberCurrencyAccount.createNew(GUILD_ID, USER_ID).withAdjustedBalance( expectedBalance);
            when(cacheService.get(CACHE_KEY, Long.class)).thenReturn(Optional.empty());
            when(accountRepository.findOrCreate(GUILD_ID, USER_ID)).thenReturn(account);
