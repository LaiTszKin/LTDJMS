...
/**
 * Contract tests for currency configuration operations.
 * Verifies that configuration responses conform to the OpenAPI contract.
 */
class CurrencyConfigContractTest {
    ...
    void currencyNameShouldRespectMaxLength() {
        ...
        GuildCurrencyConfig config = new GuildCurrencyConfig(
                TEST_GUILD_ID, maxLengthName, "🪙", now, now);
    ...
    void withUpdatesWithNullShouldPreserveExisting() {
        // Given
        Instant now = Instant.now();
        GuildCurrencyConfig original = new GuildCurrencyConfig(
                TEST_GUILD_ID, "Gold", "💰", now, now);
