...
LTDJMS 以 Discord 互動為入口，透過 Dagger 組裝 guild 級經濟、商品、派單與 AI 模組，將狀態寫入 PostgreSQL、用 Redis 做快取，並在需要時串接 ECPay 與內建的付款後處理背景流程。
...
| `src/main/java/ltdjms/discord/shop` | 商店顯示、貨幣購買、法幣訂單、ECPay callback、背景 worker、補償查單與通知 |
...
5. 建單成功後，Discord 會先私訊買家訂單編號、付款期限與逾期取消提醒，並在到期後以 terminal state 保留可查詢的逾期結果
6. callback 只做驗證、解密與 `PAID` 狀態冪等落庫
7. `FiatOrderProcessingScheduler` 週期性拉起背景 worker，補做買家通知、管理員通知、商品獎勵與 fulfilled 標記
8. `FiatPaymentReconciliationService` 會對仍是 `PENDING_PAYMENT` 的訂單做官方查單補償，回收遺失 callback 的 paid transition

### 3. AI Chat / AI Agent

1. `AIChatMentionListener` 先過濾 guild、bot、DM 與未 mention 訊息
2. 之後以 routing decision matrix 決定 `AGENT_ROUTE`、`AI_CHAT_ROUTE` 或 `DENY`
3. Agent 啟用時直接走 Agent 工具鏈，不需要先符合一般 AI allowlist
4. 非 Agent 頻道仍使用既有 AI allowlist / category allowlist 語意
5. AI 回應可經過 Markdown 驗證與切段
6. Agent 相關設定、記憶與工具執行紀錄由 `aiagent/` 管理
...
- ECPay callback、post-payment worker 與 reconciliation 都有明確的 claim / mark / retry 機制，不能隨意改順序
- `DomainEventPublisher` 是同步分發，listener 變慢會直接拖慢主流程
- 面板互動高度依賴既有 custom ID / modal ID / session 管理
- AI 頻道白名單與 AI Agent 啟用狀態是兩套設定，不要混為一談；mention routing 先看 Agent gate，再看 AI allowlist
