...
- `APP_PUBLIC_BASE_URL` 與 `ECPAY_*`：若要啟用綠界超商代碼付款
...
- `nginx`：repo 內管理的公開 HTTP ingress，代理到 bot 內嵌 callback server
...
- 若要在 Compose 自架模式啟用綠界 callback，通常只要設定 `APP_PUBLIC_BASE_URL`，不需要自己再填 `127.0.0.1` 之類的 bind host。
