...
- [ ] CL-SETUP-01 New `setup-env` creates or updates `.env` through interactive prompts
  ...
  - Test result: `NOT RUN`
  - Notes (optional): use fixture `.env` files and scripted stdin

- [ ] CL-SETUP-02 Existing secrets survive interactive setup unless explicitly replaced
  ...
  - Test result: `NOT RUN`
  - Notes (optional): verify `.env.bak` plus unchanged secrets

- [ ] CL-SETUP-03 `update-env` preserves existing non-interactive sync semantics
  ...
  - Test result: `NOT RUN`
  - Notes (optional): compare before/after fixture files against current `sync-env.sh` behavior
...
- Remaining applicable items: all implementation, fixture tests, and operator docs
- Notes: spec drafted with naming clarification applied; waiting for explicit approval before code changes

### Completion Record 2: Legacy command rename rollout
- Requirement mapping: `R2.3 / Task 1 / CL-SETUP-03`
- Completion status: deferred
- Remaining applicable items: Makefile rename, compatibility notes, README updates
- Notes: rename decision is confirmed by user, but implementation is intentionally deferred until approval
