...
- 1. [ ] 定義命令角色與入口
  - 1.1 [ ] 規劃新的 `setup-env` 為互動式引導入口
  - 1.2 [ ] 將現有同步流程改名為 `update-env`
  - 1.3 [ ] 確認 Makefile / 使用說明的入口名稱與行為一致
...
- 2. [ ] 新增互動式腳本與備份/寫入邏輯
  - 2.1 [ ] 建立 `.env` 不存在時的初始化與 `.env.bak` 備份流程
  - 2.2 [ ] 實作公開 domain / TLS email / callback 策略的提示與正規化
  - 2.3 [ ] 僅覆寫使用者確認的欄位，保留未觸及的既有 secrets
...
- 3. [ ] 補腳本驗證與摘要輸出
  - 3.1 [ ] 覆蓋新建 `.env`、保留既有值、取消流程與非 TTY 行為
  - 3.2 [ ] 驗證 `update-env` 仍保留原本同步語意
  - 3.3 [ ] 更新 operator 入口說明與剩餘人工步驟提示
