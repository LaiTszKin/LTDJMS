...
- 1. [ ] 更新 Compose ingress service
  - 1.1 [ ] 將 `nginx` service 替換為 `caddy` service，暴露 `80` / `443`
  - 1.2 [ ] 新增 Caddy data / config volume，保留 ACME 與 runtime 狀態
  - 1.3 [ ] 保持與 bot 共用 network namespace，延續 loopback proxy 模式
...
- 2. [ ] 建立 Caddy ingress 設定
  - 2.1 [ ] 定義公開 domain、ACME email 與 proxy target 的配置方式
  - 2.2 [ ] 代理 `/` 與 `ECPAY_CALLBACK_PATH` 到 `127.0.0.1:8085`
  - 2.3 [ ] 明確保留 callback server loopback bind，避免 public bind regression

## **Task 3: Validate deployment contract**

對應 `R1.x`, `R2.x`，核心目標是驗證 Compose/Caddy 設定可展開、回歸風險有覆蓋、且 operator 能看出 TLS 依賴條件。

- 3. [ ] 補 deployment 驗證與回歸檢查
  - 3.1 [ ] 以 `docker compose config` 驗證 service / volume / env wiring
  - 3.2 [ ] 補最小 regression coverage 或明確記錄 `N/A`
  - 3.3 [ ] 驗證文件與範本已反映 domain / TLS 前提
