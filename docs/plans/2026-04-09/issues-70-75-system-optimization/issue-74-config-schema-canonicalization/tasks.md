...
- 1. [ ] 收斂 `EnvironmentConfig` 與 packaged defaults
  - 1.1 [ ] 明確列出 canonical config path 與 env→config 映射
  - 1.2 [ ] 決定 `application.properties` 為唯一 packaged defaults，並移除或降級 `application.conf` 的獨立 schema 角色
...
- 2. [ ] 更新開發文件與說明範例
  - 2.1 [ ] 修正 `docs/development/configuration.md` 中對 packaged defaults 與 key namespace 的描述
  - 2.2 [ ] 若存在 compatibility shim / legacy note，文件需清楚標示其非 canonical 性質

## **Task 3: 補齊 drift regression tests**

對應 `R3.1-R3.3`，核心目標是讓未來若有人再次引入平行 schema，測試能第一時間失敗。

- 3. [ ] 擴充 shared config 測試
  - 3.1 [ ] 驗證 canonical packaged defaults 檔與主要 key 的 fallback 行為
  - 3.2 [ ] 驗證 `.env`、packaged defaults、built-in defaults 的順序與文件聲明一致
