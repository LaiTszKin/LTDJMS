...
- 1. [ ] 調整 `EcpayCvsPaymentService` 與對應測試
  - 1.1 [ ] 讓 `buildCallbackReturnUrl()` 回傳 operator 設定值，不再追加 `token`
  - 1.2 [ ] 更新既有單元測試與相關文件敘述，移除 query-token 假設
...
- 2. [ ] 收斂 `EcpayCallbackHttpServer` 授權與啟動判斷
  - 2.1 [ ] 移除 query-token 驗證邏輯
  - 2.2 [ ] 在 `ECPAY_STAGE_MODE=true` + public bind 時於啟動階段 fail closed
...
- 3. [ ] 擴充 callback 服務與 HTTP server 測試
  - 3.1 [ ] 覆蓋 production legit callback、duplicate callback、unpaid callback 與 validation failure
  - 3.2 [ ] 覆蓋 stage/public 啟動拒絕與無 query token 的新行為
