...
**Requirements**:
- [ ] R1.1 貨幣購買與法幣付款完成都必須透過同一個 handoff boundary 寫入 dispatch durable state
- [ ] R1.2 handoff 成功前不得把「護航已交接」視為完成
- [ ] R1.3 handoff 必須具備冪等保護，避免重複 callback / worker 重跑時建立重複 dispatch 訂單
...
**Requirements**:
- [ ] R2.1 `EscortDispatchOrder` 或等價模型必須保存 source order reference、source product reference、escort option snapshot 與建立來源
- [ ] R2.2 後續 dispatch UI / DM /審計資訊應從 durable record 讀取，而不是重新拼裝一次性訊息
- [ ] R2.3 若來源商品後續被編輯或刪除，既有 dispatch 訂單的執行上下文不得消失
...
**Requirements**:
- [ ] R3.1 admin DM / panel 提醒只可在 durable handoff 成功後送出
- [ ] R3.2 handoff 失敗時必須保留可重試或可診斷狀態，而不是只留下一次 warn log
- [ ] R3.3 文件必須明確把 `/dispatch-panel` 的手動開單與商品觸發的自動開單區分為兩種入口、同一個 aggregate
