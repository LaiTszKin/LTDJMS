...
**Requirements**:
- [x] R1.1 mention routing 已先判定 `agentEnabled`，並把 Agent 路徑視為獨立可用條件
- [x] R1.2 Agent 啟用為 `true` 時，即使 `AIChannelRestrictionService` 回傳 `false`，仍允許 Agent 路徑繼續
- [x] R1.3 日誌與除錯訊息已明確區分「Agent 放行」與「AI Chat 白名單放行」兩種決策來源
...
**Requirements**:
- [x] R2.1 非 Agent 頻道仍遵守現有 `AIChannelRestrictionService` allowlist 規則
- [x] R2.2 DM、bot 訊息、未 mention 訊息的忽略行為維持不變
- [x] R2.3 category allowlist 與 channel allowlist 的解析規則未因 routing 重構而改變
...
**Requirements**:
- [x] R3.1 `panel` 相關說明與 facade 語意已明確標示 Agent 頻道可獨立於 AI 白名單啟用
- [x] R3.2 單元測試已新增 Agent-only channel case，並保留 non-agent allowlist regression coverage
- [x] R3.3 文件中的流程圖與 feature 說明已對齊新 routing 規則

## Error and Edge Cases
- [x] Agent 啟用但 `agentConfigService` 暫時不可用時，已有可觀測的 fallback / deny 行為，而不是誤放行
- [x] category allowlist 命中但 channel 未啟用 Agent 時，不會誤走 Agent 路徑
- [x] 同一頻道同時在 AI 白名單且 Agent 啟用時，已有穩定且可文件化的 routing 優先順序
- [x] 私有 thread / forum thread 的 restriction channel ID 解析未被此次重構破壞
- [x] 重複 mention、空訊息預設 greeting 與 reasoning / markdown streaming 行為未被 gating 重構破壞
