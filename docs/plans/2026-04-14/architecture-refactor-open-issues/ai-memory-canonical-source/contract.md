...
- Why It Matters: 本地 `SimplifiedChatMemoryProvider`、`PersistentChatMemoryProvider` 都在實作相同抽象，必須只有一條 runtime path 為 canonical owner
- Invocation Surface:
...
- Verification Plan:
  ...
  - Planned coverage: `UT-chat-memory-canonical-path`, `DOC-review`, migration/deprecation tests
  - Evidence notes: 官方文件把 ChatMemoryProvider 定位為 runtime conversation memory 的提供者，而非模糊的歷史存放集合
