...
- Verification Plan:
  ...
  - Evidence notes: ECPay 文件描述付款方式與付款期限資訊為建單輸出的核心部分

### Dependency 2: ECPay QueryTradeInfo / 補償查單契約
- Type: `API / payment platform`
- Version / Scope: `Not fixed`
- Official Source: `https://developers.ecpay.com.tw/16538/`
- Why It Matters: reconciliation 只能對仍可付款的訂單查單，且需與本地 expiry transition 協調
- Invocation Surface:
...
- Failure Contract:
  ...
  - Caller obligations: 僅在仍有效的 pending order 上重試；避免對 terminal state 重複查單
...
- Verification Plan:
  ...
  - Planned coverage: `IT-reconciliation-excludes-expired`, `UT-paid-vs-expired-race`
